...
public class RaceDeformer {
    ...
    private float[]? ResolveDeformation( PbdFile.Deformer deformer, string name ) {
        ...
        var boneNode = _skeleton.Joints.First( x => x.Name == name );
    ...
    public Vector3? DeformVertex(PbdFile.Deformer deformer, string name, Vector3 origPos)
    {
        if (deformer.BoneNames == null)
        {
            return null;
        }
        
        var matrix = ResolveDeformation(deformer, name);
        if (matrix != null)
        {
